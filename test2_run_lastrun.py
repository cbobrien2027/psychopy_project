...
>>>>>>> b957db91e14665abe35290ef2ee16c9e43467092
...
>>>>>>> b957db91e14665abe35290ef2ee16c9e43467092
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('C:/Users/prami/OneDrive/Desktop/Conditions_Spreadsheet.csv'),
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            if circle1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > circle1.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    circle1.tStop = t  # not accounting for scr refresh
                    circle1.tStopRefresh = tThisFlipGlobal  # on global time
                    circle1.frameNStop = frameN  # exact frame index
            ...
            if circle2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > circle2.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    circle2.tStop = t  # not accounting for scr refresh
                    circle2.tStopRefresh = tThisFlipGlobal  # on global time
                    circle2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'circle2.stopped')
                    # update status
